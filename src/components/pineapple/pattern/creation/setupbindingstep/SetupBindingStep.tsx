...
import PatternBinding from "./PatternBinding";
import { usePatterns } from "../../../../../queries/patterns";
import SelectSOHeaders from "./SelectSOHeaders";
...
const SetupBindingStep = (props: any) => {
  ...
  const [selectedSO, setSelectedSO] = useState<Header[]>(new Array<Header>());
  const [hasPendingSOBindings, setHasPendingSOBindings] = useState<boolean>(false);
  ...
  const handleSOHeaderChange = (header: Header, isSelected: boolean) => {
    if (isSelected) {
      const currentSoHeaders = selectedSO.map((soHeader) => ({ ...soHeader }));
      currentSoHeaders.push(header);
      setSelectedSO(currentSoHeaders);
    } else {
      setSelectedSO(selectedSO.filter((soHeader) => soHeader.headerId !== header.headerId));
    }
  };
  ...
  useEffect(() => {
    if (!!headersLookup.data) {
      ...
      setSoHeaders(headersLookup.data.filter((soHeader: Header) => soHeader.canSetAsSO));
      if (initialState.soHeaders?.length > 0) {
        const soHeadersArr = initialState.soHeaders.split(",");
        if (soHeadersArr.length > 0) {
          const filteredHeaders = headersLookup.data.filter((d:Header) => soHeadersArr.includes(d.headerId.toString()));
          console.log("initial satte", initialState, soHeadersArr, filteredHeaders, headersLookup.data);
          setSelectedSO(filteredHeaders);
        }
      }
    }
  }, [headersLookup.data]);
  ...
  useEffect(() => {
    if (initialState?.bindings.length > 0) {
      const selectedHeaderIds = initialState?.bindings.map((initialHeader: any) => initialHeader.headerId);
  ...
  useEffect(() => {
    const currentBindingHeaderIds = bindings.map((binding) => binding.headerId);
    const pendingSOBindings = selectedSO.filter((s) => !currentBindingHeaderIds.includes(s.headerId) && s.valueSource === ValueSourceEnum.Dynamic);

    console.log(pendingSOBindings, "Pending SO Bindings");
    setHasPendingSOBindings(pendingSOBindings.length > 0);
  }, [selectedSO]);
  ...
  return (
    <>
      <Box display="flex" justifyContent="space-between">
            ...
            {headersLookup.loading ? (
              ...
              <>
                <Box component="div" padding={3}>
                  <SelectSOHeaders headers={soHeaders} soHeaderChanged={handleSOHeaderChange} selectedSO={selectedSO} />
                </Box>
                <PatternBinding
                  key={componentKey}
                ...
                />
                {hasPendingSOBindings && (
                  <Box component="div" padding={3}>
                    <Typography variant="body1" color="error">
                      Please set a binding for all Selected SO Headers.
                    </Typography>
                  </Box>
                )}
              </>
        ...
        <div className={classes.headerItemsContainer}>
          {bindings?.length <= 0 ? (
            ...
            bindings
              .filter((binding) => binding.header.valueSource === ValueSourceEnum.Dynamic)
              ?.map((binding) => <HeaderItem binding={binding} key={binding.headerId} onEdit={handleEditBinding} onRemove={handleRemoveBinding} />)
          )}
        </div>
      </Box>
      ...
      <Button variant="contained" color="primary" disabled={hasPendingSOBindings} onClick={() => handleNext(bindings, selectedSO)}>
        Next
      </Button>
    </>
  );
